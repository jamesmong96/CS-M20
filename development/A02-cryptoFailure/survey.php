<?php
    ...
    if (isset($_POST["code"]) && isset($_POST["firstName"]) && isset($_POST["lastName"])) {
        ...
        $sql = $conn->prepare("SELECT * from users WHERE firstName=? AND lastName=?;");
        $sql->bind_param("ss", $first_name, $last_name);
        $sql->execute();
        ...
        // if the identity is valid, i.e. id is set
        if ($id != "-1") {
                                ...
                                if ($result->num_rows > 0) {
                                    while($row = $result->fetch_assoc()) {
                                        ...
                                        if ($row["responses"] == "0") {
                                            ...
                                            $response = "<small>Survey Completed</small>";
                                        }
